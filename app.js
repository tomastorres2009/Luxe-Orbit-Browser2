...
function syncSearchInputs(value = '') {
  ['omniboxInput', 'heroSearchInput'].forEach(id => {
...
function engineSelectHint(engine = getPrefs().engine) {
  const label = ENGINES[engine] || 'Google';
  const heroName = document.getElementById('heroEngineName');
  const heroLogo = document.getElementById('heroEngineLogo');
  if (heroName) heroName.textContent = label;
  if (heroLogo) {
    const meta = engineMeta(engine);
    heroLogo.className = `engine-logo ${meta.className} large`;
    heroLogo.textContent = meta.mark;
  }
}
...
function renderSearchPanel(query) {
  ...
  $('#footerStatus').textContent = 'Búsqueda preparada';
  setEngineVisual(engine, 'search');
  $('#searchScreen').innerHTML = `
    <div class="browser-like">
      ...
      <div class="result-launch">
        <div class="launch-copy">
          <div class="engine-alert">
            <div>
              <strong>Tu búsqueda ya está conectada al navegador</strong>
              <small>La barra salió del centro y pasó arriba. Si un motor bloquea la vista interna, este navegador avisa y prueba con otro.</small>
            </div>
            <span class="tiny-pill">Auto motor: ${getPrefs().autoEngineFallback ? 'Sí' : 'No'}</span>
          </div>
          <h2>Buscar “${escapeHtml(query)}”</h2>
          <p>Desde aquí puedes abrir resultados dentro, en flotante o por fuera. En modo inteligente, al escribir en la barra principal se intenta abrir automáticamente dentro del navegador.</p>
          <div class="launch-actions">
            <button class="soft-btn primary" data-search-action="inside" data-engine="${engine}">Abrir dentro ahora</button>
            <button class="soft-btn" data-search-action="float" data-engine="${engine}">Abrir flotante</button>
        ...
        <aside class="launch-card">
          <div class="mini-item">
            ...
            <small>${escapeHtml(ENGINES[engine])}</small>
          </div>
          <div class="mini-item">
            <strong>Modo</strong>
            <small>${escapeHtml(labelForMode(state.searchMode))}</small>
          </div>
          <div class="mini-item">
...
function attemptNextEngine(reason = 'timeout') {
  ...
  toast('Cambio automático', `Este motor se bloqueó o tardó demasiado. Ahora se intenta con ${ENGINES[nextEngine]}.`);
  animateProgress();
...
function openEntry(entry, push = false) {
  ...
  renderTabs();
  syncSearchInputs(entry.kind === 'home' ? '' : entry.value);
...
function navigate(raw) {
  ...
  if (entry.kind === 'search') {
    ...
    if (prefs.openBehavior === 'inside' || prefs.openBehavior === 'smart') {
      toast('Buscando', 'Intentando abrir resultados dentro del navegador.');
...
function setupRecognition() {
  ...
  if (!API) {
    ...
    if (heroVoice) heroVoice.title = 'La búsqueda por voz no está disponible aquí';
    return;
  }
  const rec = new API();
  rec.lang = 'es-CO';
  ...
  rec.onstart = () => {
    ...
    const heroVoice = document.getElementById('heroVoiceBtn'); if (heroVoice) heroVoice.classList.add('active');
    toast('Escuchando', 'Habla ahora para buscar.');
  };
  rec.onend = () => {
    state.listening = false;
    $('#voiceBtn').classList.remove('active');
    const heroVoice = document.getElementById('heroVoiceBtn'); if (heroVoice) heroVoice.classList.remove('active');
  };
  rec.onerror = () => {
    state.listening = false;
    $('#voiceBtn').classList.remove('active');
...
function wireStaticEvents() {
  ...
  $('#heroSearchForm').addEventListener('submit', e => { e.preventDefault(); hideSuggestions(); navigate($('#heroSearchInput').value); });
  $('#omniboxInput').addEventListener('input', e => { syncSearchInputs(e.target.value); showSuggestions(e.target.value); });
  $('#heroSearchInput').addEventListener('input', e => syncSearchInputs(e.target.value));
  ...
  $('#heroSearchBtn').addEventListener('click', () => $('#heroSearchInput').focus());
  $('#heroVoiceBtn').addEventListener('click', toggleVoiceSearch);
